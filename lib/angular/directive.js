      ...
      self.done = function(isValid) {
        ...
        if (isValid) {
          ...
          if (!self.model.id && self.model.id !== 0) {
            self.model.steps = [];
            mediator.publish('wfm:workflow:created', removeAngularVars(self.model));
          }  else {
            mediator.publish('wfm:workflow:updated', removeAngularVars(self.model));
          }
        }
      };
    ...
    , controller: function($scope) {
      ...
      self.done = function(isValid) {
        ...
        if (isValid) {
          ...
          mediator.publish('wfm:workflow:updated', removeAngularVars($scope.workflow));
        }
      };
...
/**
 * Removes variables added by angular from the workflow
 * @param {Object} workflow
 */
function removeAngularVars(workflow) {
  return JSON.parse(angular.toJson(workflow)); //remove generated angular variables
