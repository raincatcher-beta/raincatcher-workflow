...
function WorkflowMediatorService(mediator, config) {
  ...
  this.workflowSyncSubscribers = new MediatorTopicUtility(mediator).prefix(CONSTANTS.SYNC_TOPIC_PREFIX).entity(CONSTANTS.WORKFLOW_ENTITY_NAME);
}
...
WorkflowMediatorService.prototype.getWorkorderSummary = function(workorderId) {
  ...
  return this.readWorkorder(workorderId).then(function(workorder) {
    return q.all([self.read(workorder.workflowId), self.getResultByWorkorderId(workorderId)]).then(function(allResults) {
      var workflow = allResults[0];
      var result = allResults[1];
